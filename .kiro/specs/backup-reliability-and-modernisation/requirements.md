...
5. THE `maxAllowedLimit` value SHALL become a template parameter with a default of **200**, its current value, and an allowed range up to the API maximum of 1000. THE default SHALL NOT be raised speculatively. THE API permits 1000 and the library defaults to it, but the binding constraint is not the page limit: Contentful enforces a hard response-size ceiling of 7,340,032 bytes, and the documented remedy for exceeding it is to LOWER this value. BECAUSE the ceiling is a function of entry size rather than entry count, a space with large rich-text entries reaches it well below 1000 entries, and 200 is therefore to be treated as an empirically-derived working value rather than an oversight.
6. THE Backup_Lambda SHALL handle a response-size error adaptively: on encountering it, halve the effective page limit and retry the affected request, down to a documented floor, before failing. THIS makes a higher configured value degrade gracefully instead of failing the export, so the parameter can be tuned upward safely.
7. THE commissioning export authorised by criterion 0.7 SHALL record the largest page limit that succeeds for this space, and the design SHALL state whether raising the default is warranted. WITHOUT that measurement the reduction in request count is speculative, and the cost of guessing wrong is a thrown export that is redelivered and consumes MORE quota than the smaller pages would have.
8. THE Backup_Manifest SHALL record that the archive is published-state only.
9. THE test that asserts `includeDrafts: true` SHALL be rewritten to assert published-state configuration, citing this requirement.
10. THE Backup_Lambda documentation SHALL state that unpublished editorial work is not captured.
11. THE design SHALL state the effect of the published-state switch on Contentful consumption, including that entries and assets move from the Management API to the Content Delivery API, which carries a separate rate-limit allowance — so the switch shifts the heaviest part of the load off the endpoint that editorial work and Amplify builds compete for.
...
6. THE CloudFormation_Template SHALL record the rationale for `MemorySize` as an inline comment. THE current value of 450 is not a tuned value but a stale one the workload has outgrown: the largest current backup consumes ~950 MB, so the existing function is OOM-killed on its largest runs. THE interim value SHALL sit comfortably above that measured peak (not at ~1 GB, which leaves no headroom over 950 MB), because the export result object and the archiver's in-memory work dominate regardless of the streamed upload.
...
11. THE design SHALL state the tested size and duration envelope, and the documentation SHALL record it with its failure symptom. BECAUSE the largest current backup (~950 MB, and an unmeasured but large fraction of the 900 s timeout) is already close to Lambda's ceilings on a growing space, the commissioning measurement SHALL record actual peak memory and duration and the decision record SHALL flag whether either exceeds ~70% of its ceiling (10240 MB memory, 900 s on-demand timeout) — the signal that the space will outgrow Lambda and that a different delivery path (Step Functions or ECS) must be revisited before it fails, not after.
...
3. EVERY queue SHALL declare a resource policy denying non-TLS access. A criterion naming only two queues would leave the Terminal_Queue uncovered, so the obligation is expressed per queue rather than by count.
