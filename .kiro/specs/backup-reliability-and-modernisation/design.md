...
`MemorySize` is 450 today with no recorded justification, and `EphemeralStorageSize` is unset (512 MB default) while the function writes an export plus its archive to `/tmp`. The final values come from the **one-time commissioning export** (criterion 0.7) — but that export runs on the deployed function, so the design deploys with **interim** values first (`MemorySize` 1024 MB, `EphemeralStorageSize` 2048 MB), the export measures the real envelope, and a **second stack update** sets the finals. This two-pass sequence is stated so a task list can order it. Streaming is what makes the envelope tractable: memory scales with the archiver window, so `/tmp` — a declared number — is the binding constraint.
